...
`.github/workflows/experiment-saref.yml` runs `detect` for real (full backfill, every entity in `tracked-entities.json`) on every push. Any candidate not already sitting in `src/` is copied there and committed straight to `main` with `[skip ci]` -- this is safe to automate because it only ever records release *facts* (what the feed says exists), never an ontology edit; the actual `ontologies/*.ttl` change still only ever reaches `main` via the usual candidate-patch -> combined PR -> human-review path. Only newly-recorded candidates get an LLM request built this run, so an evidence file already in `src/` (still pending review, or already merged) is never silently reprocessed.
