...
def create_combined_pr(
    ...
    """commits: [{"message": str, "changed_files": [path relative to repo_root, ...]}, ...].
    ...
    for commit in commits:
        print(f"  commit: {commit['message'].splitlines()[0]} ({len(commit['changed_files'])} file(s))")
    print(f"would open PR '{pr_title}' targeting '{base}'")
    ...
    run(["git", "-C", str(repo_root), "checkout", "-b", branch, base])
    for commit in commits:
        for rel_path in commit["changed_files"]:
            run(["git", "-C", str(repo_root), "add", rel_path])
...
def _combine_open_pr(args: argparse.Namespace) -> None:
    ...
    for manifest_path in manifest_paths:
        ...
        for patch in manifest.get("patches", []):
            ...
            if not source.exists():
                ...
                continue
            shutil.copyfile(source, args.repo_root / target_rel)
        ...
        commits.append({"message": manifest["commit_message"], "changed_files": sorted(set(changed_files))})
    ...
    pr_title = args.title or f"research: combined update from {', '.join(included)}"
    pr_body = args.body or (
        "Combined PR from the parallel research pipeline. One commit per experiment:\n\n"
