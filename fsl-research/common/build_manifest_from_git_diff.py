...
def main() -> None:
    ...
    parser.add_argument("--message", required=True, help="One-line summary of the change, for the commit message")
    parser.add_argument("--details", default="", help="Optional extra body text for the commit message")
    ...
    write_json(args.out_dir / "manifest.json", {
        ...
        "commit_message": format_review_commit_message(
            experiment=args.experiment, summary=args.message, details=args.details,
        ),
        "patches": patches,
    })
    if patches:
