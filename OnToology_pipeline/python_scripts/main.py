...
if __name__ == '__main__':
    ...
    merged_str = generate_resources.merge(MAIN_DIR / "fsl.ttl", module_paths)
    FSL_MERGED_TTL.write_text(merged_str, encoding='utf-8')
    generate_resources.convert_merged_to_owl(FSL_MERGED_TTL, FSL_MERGED_OWL)
    send_oops_request.scan_ontology(str(FSL_MERGED_OWL), str(OOPS_RESPONSE / "oops_report.xml"))
    reqs = build_batch_request.build_batch_requests(
            merged_ontology_path=str(FSL_MERGED_TTL),
            oops_xml_path=str(OOPS_RESPONSE / "oops_report.xml"),
            pitfall_ids=[i for i in range(1, 42)],
            fsl_summary_path=str(LLM_PROMPTING / "system_message/FSLsummary.md")
        )
